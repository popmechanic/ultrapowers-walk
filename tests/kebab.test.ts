...
// Leg (a) [M1], the named failure modes: a result that keeps an upper-case
// letter, or drops the boundary ('helloworld'), fails.
test('leg (a) [M1]: no upper-case survives and the boundary is not dropped', () => {
  const out = kebabCase('helloWorld')
  expect(out).not.toBe('helloworld')
  expect(out).toBe(out.toLowerCase())
  expect(kebabCase('HelloWorldAgain')).toBe(kebabCase('HelloWorldAgain').toLowerCase())
})

// Leg (b) [M2]: an underscore or a run of whitespace is a word boundary too,
// and an already-kebab input is unchanged.
...
// Leg (b) [M2], the named failure modes: a doubled `--` or a kept underscore
// fails.
test('leg (b) [M2]: no doubled hyphen and no underscore is kept', () => {
  for (const input of ['snake_case_in', 'two words', 'already-kebab']) {
    const out = kebabCase(input)
