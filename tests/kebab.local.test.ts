...
// Leg (b) [M2]: an underscore or a run of whitespace is a word boundary too, and
// an already-kebab input is unchanged. A doubled `--` or a kept underscore fails.
...
test('module leg [M5]: this file imports kebabCase from ../src/kebab', () => {
  const ownSource = readFileSync(join(repoRoot, 'tests', 'kebab.local.test.ts'), 'utf8')
