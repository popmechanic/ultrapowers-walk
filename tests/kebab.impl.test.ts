...
// [M5] The implementer's own checks import `kebabCase` from `../src/kebab`.
// The graded exam lives at `tests/kebab.test.ts` and is written by a peer.
...
// Module leg [M5]: `src/kebab.ts` exports `kebabCase` at top level and no other
// module of `src/` defines it.
...
// The module leg also requires that the graded exam import from `../src/kebab`.
// That file is the peer's to write, so it is not asserted here.
